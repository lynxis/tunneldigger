...
test_nose() {
  ...
  cd $WORKSPACE/tests/
  if ! CLIENT_REV=$old_rev SERVER_REV=$new_rev nosetest3 test_nose.py ; then
    fail "while running test_nose cli <> server.\nclient: '$old_rev'\nserver: '$new_rev'"
  fi
  if ! CLIENT_REV=$new_rev SERVER_REV=$old_rev nosetest3 test_nose.py ; then
    fail "while running test_nose cli <> server.\nclient: '$new_rev'\nserver: '$old_rev'"
  fi
}
...
test_usage() {
  for i in seq 1 5; do
    if ! nosetest3 test_usage.py ; then
